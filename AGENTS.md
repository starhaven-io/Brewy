...
- `.github/workflows/link-check.yml`: weekly README, CONTRIBUTING, and SECURITY link check.
...
7. Preserve derived-state invalidation when changing installed package mutations, reverse dependencies, leaves, pinned packages, or outdated merges. Package and group equality must include their values so Observation sees metadata edits; list selection uses stable IDs.
...
12. The cask `depends_on` decoder accepts objects, absent/null values, and the legacy empty array. Reject other malformed dependency content so a failed refresh preserves the previous inventory.
13. Brewfile trust binds exact file bytes and path. Execute approved snapshots through stdin; canceled or superseded reads must not overwrite the selected Brewfile state.

## Release recovery

If publication succeeds but appcast or cask delivery fails, verify the existing
published asset and provenance before repairing the failed delivery step. Keep
the release tag and uploaded bytes immutable; rebuilding the same version is not
a recovery mechanism. A release is distributed only after its tap cask update
merges. Use the normal explicit authorization for any hosted repair.
